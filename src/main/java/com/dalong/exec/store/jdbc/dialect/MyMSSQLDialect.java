package com.dalong.exec.store.jdbc.dialect;
...
public class MyMSSQLDialect extends ArpDialect {
   ...
   public MyMSSQLDialect(ArpYaml yaml) {
      super(yaml);
   }
   ...
   public SqlNode getCastSpec(RelDataType type) {
      ...
      case VARCHAR:
         if (type.getPrecision() <= MSSQL_MAX_VARCHAR_LENGTH && type.getPrecision() != -1) {
            return getVarcharWithPrecision(this, type, type.getPrecision());
         }

         return getVarcharWithPrecision(this, type, MSSQL_MAX_VARCHAR_LENGTH);
      case TIMESTAMP:
         return new SqlDataTypeSpec(new SqlBasicTypeNameSpec(type.getSqlTypeName(), -1, -1, (String)null, SqlParserPos.ZERO), (TimeZone)null, SqlParserPos.ZERO) {
            public void unparse(SqlWriter writer, int leftPrec, int rightPrec) {
               writer.keyword("[DATETIME2]");
            }
         };
      case DOUBLE:
         return new SqlDataTypeSpec(new SqlBasicTypeNameSpec(type.getSqlTypeName(), -1, -1, (String)null, SqlParserPos.ZERO), (TimeZone)null, SqlParserPos.ZERO) {
   ...
   public void unparseCall(SqlWriter writer, SqlCall call, int leftPrec, int rightPrec) {
      ...
      if (op == SqlStdOperatorTable.TIMESTAMP_DIFF) {
         ...
         if (select.getFetch() == null || select.getOffset() != null && (Long)((SqlLiteral)select.getOffset()).getValueAs(Long.class) != 0L) {
            ...
            SqlSelect modifiedSelect = SqlSelectOperator.INSTANCE.createCall(keywords, select.getSelectList(), select.getFrom(), select.getWhere(), select.getGroup(), select.getHaving(), select.getWindowList(), select.getQualify(), select.getOrderList(), (SqlNode)null, (SqlNode)null, (SqlNodeList)null, SqlParserPos.ZERO);
   ...
   public JdbcSchemaFetcher newSchemaFetcher(JdbcPluginConfig config) {
      return new MSSQLSchemaFetcher(config);
   }
   ...
   protected static class MSSQLSchemaFetcher extends ArpSchemaFetcher {
      private static final Logger logger = LoggerFactory.getLogger(MSSQLSchemaFetcher.class);

      protected MSSQLSchemaFetcher(JdbcPluginConfig config) {
